...
<html>
  <head>
    ...
    ${libs.reduce((acc, item) => { return `${acc}<script type="text/javascript" src="./src/lib/${item}"></script>\n` }, '')}
    ${scripts.reduce((acc, item) => { return `${acc}<script type="text/javascript" src="./src/${item}"></script>\n` }, '')}
  </head>
  <body>
    <script>
      const client = new Client()
